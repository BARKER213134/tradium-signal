...
def _log_event_async(event_type: str, data: dict) -> None:
    """Fire-and-forget event-log в Mongo через to_thread.
    Не блокирует event loop. Используется внутри async-функций вместо
    sync _events().insert_one() — раньше каждый sync insert вешал tick
    при медленном Atlas (см. memory: no_sync_mongo_in_event_loop)."""
    def _sync():
        try:
...
async def _check_reversal_flip():
    ...
    try:
        ...
        # Записываем event для маркера на графиках (через to_thread)
        def _save_rev_event():
...
async def _send_cryptovizor_alert(signal: Signal, pattern: str, current_price: float,
    ...
    # Маячок что функция вызвана (fire-and-forget через to_thread)
    _log_event_async("cv_alert_called",
    ...
    if resp is None:
        ...
        _log_event_async("cv_alert_error",
        ...
        if resp.get("ok"):
            ...
            _log_event_async("cv_alert_sent",
                             {"signal_id": signal.id, "pair": signal.pair,
                ...
                if st_passed:
                    async def _fire_and_forget_alert(sig_obj, pat, cp, _s1, _r1, _png):
                        try:
                            ...
                            _log_event_async("cv_alert_timeout_global",
...
async def _send_ai_signal_alert(signal, ai_result, current_price):
    ...
    # ── Маячок СРАЗУ (fire-and-forget через to_thread) ──
    _log_event_async("ai_alert_called",
                     {"signal_id": signal.id, "pair": signal.pair,
    ...
    if not sent:
        ...
        _log_event_async("ai_alert_all_failed",
                         {"signal_id": signal.id, "pair": signal.pair,
...
async def _ai_background_analysis(signal, current_price, s1, r1):
    ...
    try:
        ...
        if full_analysis:
            ...
            await asyncio.wait_for(asyncio.to_thread(_save), timeout=5.0)
