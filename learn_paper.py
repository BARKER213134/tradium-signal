...
def _open_new(db, model, now):
    ...
    for key, pair, c in sorted(cands, key=lambda x: x[2]["at"], reverse=True):
        ...
        # ⛰ 23.09: лайв берёт лонги только ОТ ДНА. 25.09 уточнено по школе
        # (разложил ⛰ по причинам с 23.09 17:52): только широта>60 при
        # красной/нулевой серии ×2 → WR 77 +6.47 (n=35), а широта>60 +
        # зелёная серия ≥5 → WR 60 +3.90 (n=10). Бэктест 45д согласен
