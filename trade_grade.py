...
EDGE = {
    ...
    # конфлюенсы: ПЕРЕСЧЁТ 26.07 на свежих 90д (старый квартальный
    # бэктест давал 5plus LONG +1.04/A — на 90д это −0.67; лонги C,
    # шорты рабочие только со светофором)
    ('confluence_5plus', 'LONG', 'ВСЕ'): (332, 34.0, -0.67, 14.2),
    ('confluence_5plus', 'SHORT', 'ВСЕ'): (477, 48.6, 0.22, 12.8),
    ('confluence_lo', 'LONG', 'ВСЕ'): (3731, 34.8, -0.53, 17.3),
    ('confluence_lo', 'SHORT', 'ВСЕ'): (4345, 45.3, 0.15, 15.6),
    ...
    # 🌋 blowoff-вершина (год, 25.07: сетка SHORT −10/+5/96ч)
    ('blowoff', 'SHORT', 'ВСЕ'): (3748, 40.0, 0.59, 31.8),
    ...
    # 💨 тонкий памп (90д, 25.07: рост >2.2× медианы на объёме <1.25×
    # медианы, сетка SHORT −10/+5/96ч; строгий срез vol<1.0: 581/51.3/+1.36;
    # случайный шорт того же окна: 39.8 / −0.22)
    ('thin_pump', 'SHORT', 'ВСЕ'): (1137, 49.0, 0.98, 24.4),
    # 💎 дно+покупатель (90д, 25.07: имбаланс покупки >=65% на новом
    # 48ч-лоу, объём >1.5× медианы; сетка LONG +10/−5/96ч)
    ('floor_buy', 'LONG', 'ВСЕ'): (216, 52.3, 0.67, 17.1),
}
...
def annotate_pro(items: list, extra_ctx: dict = None) -> None:
    ...
    for it in items:
        try:
            ...
            if d == "LONG":
                if phase == "SHORT":
                    if st4f:
                        ...
                        # 🛟 капитуляция исключением НЕ стала: год +0.82 в 🔴,
                        # но помесячно режимно (сен −4.03, июнь −2.13, окт
                        # +1.95, фев +2.20) и последний квартал −1.26 —
                        # в затяжном красном лонги дна сливают
            ...
            # заработанная звезда остаётся (даунгрейда нет — как в
            # star_upgrade): живой пересчёт терял ⭐, когда монета уже
            # уехала из профиля (шорт слил >10%/24ч → «догон» и т.п.) —
            # 22.07 в журнале «пропали» 20 шорт-звёзд (T/ZIL/CHR).
            # При жёстком НЕТ звезду не показываем — не зовём в запрет.
            if not star and verdict != "НЕТ" and it.get("svetofor_star"):
                star = True
                checks.insert(0, "⭐ ОТБОРНЫЙ (зафиксирован при появлении "
                                 "сигнала — профиль победителей)")
            it["pro_star"] = star
