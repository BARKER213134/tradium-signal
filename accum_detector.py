                ...
                if _bo is not None:
                    ...
                    try:
                        ...
                        if store_signal(_bo, cooldown_h=24):
                            ...
                            # 🚦 TG-гейт (бэктест 04.08, 1016 закрытых 120д):
                            # нейтральная фаза EV −0.33 (половина сигналов!),
                            # 5-я+ вершина EV −0.07 → TG только фаза≠NEUTRAL
                            # и rep_seq<=4 (EV +2.11, WR 58, SL 38% против
                            # базы +0.79/50/47). Слабые — молча в журнал.
                            _ph = _bi.get("phase")
                            _rep = _bo.get("rep_seq") or 1
                            _gate = (_ph != "NEUTRAL") and _rep <= 4
                            # 💎 усиленный тир: + разгон>=20 или 🔥 горячая
                            # (EV +2.61, WR 64, SL 34%)
                            _elite = _gate and ((_bi.get("mom24") or 0) >= 20
                                                or _bo.get("hot"))
                            if not _gate:
                            ...
                            if _elite:
                                _rl += ("💎 <b>УСИЛЕННЫЙ</b> — фаза "
                                        f"{_ph or '?'} + "
                                        + ("разгон" if (_bi.get("mom24") or 0) >= 20
                                           else "🔥 горячая")
                                        + " (WR 64, EV +2.61)\n")
                            # 🔁 3-я/4-я вершина за 14д — повтор-усилитель
                            if 3 <= _rep <= 4:
                                _rl += (f"🔁 вершина №{_rep} за 14д — "
                                        f"повтор-усилитель (EV +2.98 у 3-й)\n")
                            ...
                            _tg16(f"{'🧿 ' if _e1d else ''}"
                                  ...
                                  f"— моментум сломан · фаза {_ph or '?'}\n"
                                  + _rl +
                                  f"SL {_bo['sl']:.6g} (над кульминацией, "
                                  f"+{_sl_pct:.1f}%) · TP −10% · до 96ч\n"
                                  f"<i>120д-гейт: EV +2.11/сигнал, WR 58, "
                                  f"выбивание 38%</i>")
