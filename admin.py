            ...
            while True:
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                    # 🧹 malloc_trim КАЖДУЮ минуту: вернуть ОС освобождённую
                    # glibc-память (фрагментация арен; 16:18 29.07 — первый
                    # в истории спад RSS без рестарта, рост +37→+16 МБ/мин)
                    try:
...
class SessionAuthMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if path in ("/login", "/health", "/healthz", "/api/setup-check", "/api/resonance-screenshot", "/api/resonance-status", "/api/backtest-st", "/api/backtest-st/status",
            ...
            "/api/live/confirm", "/api/inspect-msg-neighbors", "/api/debug-fetch-chart", "/api/reversal-meter", "/api/pending-clusters", "/api/backfill-clusters", "/api/pair-signals", "/api/conflicts", "/api/conflicts/check", "/api/smart-levels", "/api/ai-coin-analysis", "/api/top-picks", "/api/top-picks/backfill", "/api/claude-budget", "/api/admin/health-detail", "/api/journal/by-symbol", "/api/market-events", "/api/market-events/backfill", "/api/backtest/today") or path.startswith("/static") or path.startswith("/api/live/accounts"):
        ...
        try:
            ...
            if ms and ms.get("raw_side") == side and ms.get("raw_since"):
                ...
                if side == "SHORT":
                    ...
                    # ⚪ по возрасту (год): свежий = инерция разгона (лонги
                    # +2.4пп, шорты −7.6); 48ч+ = перегрев застарел (шорт-
                    # предвосхищение +5.4пп, лонг −10); ломается в 🔴 в 72%
                    phase_hint = ("инерция разгона — лонги слегка + (35%), шорты мертвы (32%)" if a < 8
                                  else "мёртвая зона — обе стороны без эджа" if a < 48
            ...
            if cd and cd.get("state"):
                ...
                if cs == "SHORT" and side == "LONG":
                    ch = ("🔥 разворот из красного климата — лучшие лонги года "
                          "(WR 58%, EV +2.0): бери 🟩/⭐-лонги из журнала, "
                          "стоп −5 / цель +10, не жди подтверждения климата")
                elif cs == "LONG" and side == "NEUTRAL":
                    ch = ("⚠️ перегрев 12ч остывает: НЕ докупать лонги "
                          "(худшая ячейка года −2.1) — искать шорты слабых "
                          "монет, фейд вершины EV +2.7")
                elif cs == "LONG" and side == "SHORT":
                    ch = ("фейд перегрева: шорты в плюсе (+0.9), лонги мертвы — "
                          "шорти отскоки, не лови дно")
                elif cs == "LONG" and side == "LONG":
                    ch = ("оба этажа 🟢 — движение уже выработано (согласие "
                          "этажей эджа НЕ даёт): вход только по свежему "
                          "триггеру, ничего не догонять")
                elif cs == "SHORT" and side == "SHORT":
                    ch = ("оба этажа 🔴 — шорт только отобранный (лидер "
                          "слабости 7д + светофор ДА): случайный шорт "
                          "здесь уже ~0 (+0.06)")
                elif cs == "SHORT":
                    ch = ("красный климат: лонги только на свежем 🟢-развороте "
                          "4ч (лучшая ячейка года), иначе жди или ранний шорт ⚪🔻")
                else:
                    ch = "климат 12ч нейтрален — решает 4ч-фаза и подфаза"
...
<script src="/static/footprint.js?v=15"></script>
</body></html>""")
    ...
    try:
        ...
        for s in _sts().find({
            ...
            items.append({
                ...
                "st_tier": tier,
                "mso_streak2h": s.get("mso_streak2h"),
                "validator_ok": s.get("validator_ok"),
        ...
        for n in nss.find({"created_at": {"$gte": since}, **pair_or,
            ...
            if strat == "whale":
                ...
                extra_parts.append(
                    ...
                    f"{_fri.get('risk_pct', '?')}%")
            elif strat == "rev_candle":
                _rci = n.get("indicators") or {}
                extra_parts.append(
                    f"{'пин-бар' if _rci.get('kind') == 'pin' else 'поглощение'}"
                    f" ×{_rci.get('rng_x', '?')} у зоны "
                    f"⚡{_rci.get('strength', '?')}% · RSI4h "
                    f"{_rci.get('rsi4h', '?')} · тайминг, не вход")
...
def _compute_journal_sync(_fast_only: bool = False):
    ...
    try:
        ...
        # редкие стратегии — отдельный запрос, иначе их выдавливали из
        # limit(2500) объёмные blowoff/аномалии (29.07: 18 ракет из 70)
        _seen_nss = {d["_id"] for d in _nss_docs}
        _nss_docs += [d for d in nss_col.find(
            {"created_at": {"$gte": _deep_since},
        ...
        for n in _nss_docs:
            ...
            if strat in ("st_break", "st_break4h"):
                ...
                _rci2 = n.get("indicators") or {}
                extra = (f" · разворот 4h: "
                         f"{'пин-бар' if _rci2.get('kind') == 'pin' else 'поглощение'}"
                         f" ×{_rci2.get('rng_x', '?')} среднего диапазона, "
                         f"закрытие за краем зоны ⚡{_rci2.get('strength', '?')}% "
                         f"({_rci2.get('touches', '?')}кас) · RSI4h "
                         f"{_rci2.get('rsi4h', '?')} · стоп за экстремумом "
                         f"−{_rci2.get('risk_pct', '?')}% · тайминг-сигнал "
                         f"(бэктест: эдж у лимитки от края)")
            ...
            _PRIO = {
                ...
                # 🏓 st_touch — сид из год-бэктеста (4h/12h, EV +0.61..0.90);
                # заменить честным EV трекера при n>=40
                "st_touch": (0.75, None),
                # 🧲 mso_retest — сид 16.09: 2h без гейта +0.42,
                # 4h с гейтом «тренд DOWN» +0.70; смесь ~0.5
                "mso_retest": (0.5, None),
                # 🌡 mso_obexit — сид 16.09 с гейтом «12h тренд UP»: +0.61
                "mso_obexit": (0.61, None),
                # 🤿 rsi_deepos — сид 16.09 с гейтом «1d не UP»: +0.50
                "rsi_deepos": (0.5, None),
    ...
    try:
        ...
        return {
            ...
            "description": (
                ...
                "🎯 Гейт стороны рынка: пока бейдж 🔴 SHORT (ширина <40% или "
                "40–60% при BTC ST4h DOWN) — входы на паузе (год: лонги при "
                "слабой ширине дают WR 27% при безубытке 33)."
            ),
            "enabled": enabled,
            "backtest": {
                "win_rate_pct": "65–70",
